...
int acts_fsldma_external_start0(struct dma_chan *chan)
{
	struct owl_dma_chan *atchan = to_owl_dma_chan(chan);
...
EXPORT_SYMBOL(acts_fsldma_external_start0);

int acts_fsldma_external_start1(struct dma_chan *chan)
{
	struct owl_dma_chan *atchan = to_owl_dma_chan(chan);
...
EXPORT_SYMBOL(acts_fsldma_external_start1);

int acts_fsldma_external_start2(struct dma_chan *chan)
{
	struct owl_dma_chan *atchan = to_owl_dma_chan(chan);
...
EXPORT_SYMBOL(acts_fsldma_external_start2);
/**
...
static const struct of_device_id owl_dma_of_match[] = {
    {.compatible = "caninos,labrador-dma", .data = &owl_dma_platform_data[0]},
    {}
};
...
static int __init owl_dma_probe(struct platform_device *pdev)
{
	...
	dma_cap_zero(owl_dma->dma_common.cap_mask);
	
	dma_cap_set(DMA_SLAVE, owl_dma->dma_common.cap_mask);
	...
	pr_info("DMA Controller ( %s%s%s), %d channels\n",
