...
static ssize_t error_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int cnt, tmp;
...
static int __init atm7059_link_init(void)
{
	...
	///pmu_type = atc2603c_audio_get_pmu_status();   <<<<<---------- TODO!!!!!
	
	pmu_type = PMU_NOT_USED;
