...
{
	return (port->type == PORT_OWL) ? "caninos-uart" : NULL;
}
...
static void owl_uart_early_console_write(struct console *co, const char *s, u_int count)
{
...
static int __init owl_uart_early_console_setup(struct earlycon_device *device, const char *opt)
{
...
OF_EARLYCON_DECLARE(owl, "caninos,labrador-uart", owl_uart_early_console_setup);

#define OWL_UART_CONSOLE (&owl_uart_console)

static struct uart_driver owl_uart_driver = {
	.owner = THIS_MODULE,
	.driver_name = "caninos-labrador-uart",
