...
static ssize_t regs_store(struct device_driver *drv, const char *buf, size_t count)
{
	return de_ops->write_regs(buf, count);
}
static DRIVER_ATTR_RW(regs);

static ssize_t vb_timeout_cnt_show(struct device_driver *ddp, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", de_pdata->vb_timeout_cnt);
}

static DRIVER_ATTR_RO(vb_timeout_cnt);
