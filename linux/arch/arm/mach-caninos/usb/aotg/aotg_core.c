...
struct of_device_id aotg_of_match[] = {
	{.compatible = "caninos,labrador-usb-2.0-0",.data = &aotg_data0},
	{.compatible = "caninos,labrador-usb-2.0-1",.data = &aotg_data1},
	{},
};
...
void aotg_get_dts(void)
{
	...
	of_node = of_find_compatible_node(NULL, NULL, "caninos,labrador-usb-2.0-0");
	if (of_node) {
	...
	of_node = of_find_compatible_node(NULL, NULL, "caninos,labrador-usb-2.0-1");
	if (of_node) {
...
int aotg_probe(struct platform_device *pdev)
{
	...
	if (aotg_mode[pdev->id] == HCD_MODE) {
		...
		timer_setup(&acthcd->hotplug_timer, aotg_hub_hotplug_timer, 0);
...
static int __init aotg_init(void)
{
	...
	queue_delayed_work(start_mon_wq, &start_mon_wker, msecs_to_jiffies(1000));
	return 0;	
}
