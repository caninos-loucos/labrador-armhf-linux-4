...
extern int acts_fsldma_external_start0(struct dma_chan *chan);
extern int acts_fsldma_external_start1(struct dma_chan *chan);
extern int acts_fsldma_external_start2(struct dma_chan *chan);

static inline int owl_dma_dump_all(struct dma_chan *chan)
{
	return acts_fsldma_external_start0(chan);
}

static inline int read_remain_cnt(struct dma_chan *chan)
{
	return acts_fsldma_external_start1(chan);
}

static inline int read_remain_frame_cnt(struct dma_chan *chan)
{
	return acts_fsldma_external_start2(chan);
}
