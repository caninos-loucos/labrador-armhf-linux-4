...
/*Defines for k5 clocks names */
#ifndef MODULE_CLK_I2C1

#define MODULE_CLK_I2C3          29
#define MODULE_CLK_I2C2          30
#define MODULE_CLK_I2C1          44
#define MODULE_CLK_I2C0          45
...
static int owl_i2c_probe(struct platform_device *pdev)
{
	...
	/*check if is k5 Base Address, because the clock are not get from device tree, otherwise get clock from dt*/
	switch (res->start)
	{
		case 0xB0170000:
				dev->clk = clk_get(NULL, CLKNAME_I2C0_CLK);
				mod_clk_en = MODULE_CLK_I2C0;
			break;

		case 0xB0174000:
				dev->clk = clk_get(NULL, CLKNAME_I2C1_CLK);
				mod_clk_en = MODULE_CLK_I2C1;
			break;
		
		case 0xB0178000:
				dev->clk = clk_get(NULL, CLKNAME_I2C2_CLK);
				mod_clk_en = MODULE_CLK_I2C2;
			break;
		
		case 0xB017C000:
				dev->clk = clk_get(NULL, CLKNAME_I2C3_CLK);
				mod_clk_en = MODULE_CLK_I2C3;
			break;
		//if it is k7 or k9
		default:
	...
	//k5 use module_clk_enable to enable clock
	if(mod_clk_en != 0)
		if(module_clk_enable(mod_clk_en))
			pr_err("I2C fail to enable module clk");
